...
DRIVERS = [
    ...
    {"name": "Lewis Hamilton", "skill": 9, "consistency": 9},
    {"name": "Ayrton Senna", "skill": 10, "consistency": 8}
...
TRACKS = [
    ...
    {"name": "Interlagos", "difficulty": 0.7, "speed_bias": 0.6},
    {"name": "Spa-Francorchamps", "difficulty": 0.85, "speed_bias": 0.9},
    {"name": "Monza", "difficulty": 0.4, "speed_bias": 1.0},
    {"name": "Marina Bay", "difficulty": 0.9, "speed_bias": 0.3},
    {"name": "Zandvoort", "difficulty": 0.75, "speed_bias": 0.5},
    {"name": "Circuit of the Americas", "difficulty": 0.65, "speed_bias": 0.7},
    {"name": "Red Bull Ring", "difficulty": 0.5, "speed_bias": 0.9}
]
...
def choose_driver(available_drivers: list[dict[str, int]]) -> dict[str, int]:
    ...
    while True:
        try:
            choice = int(input(f"\nSelect your driver (1-{len(available_drivers)}): "))
...
def run_championship(available_drivers: list[dict[str, int]], available_tracks: list[dict[str, float]]) -> dict[str, int]:
    ...
    TEST_VARIOUS_TIE = False
    TEST_DUAL_TIE = True 

    if TEST_VARIOUS_TIE:
        print("\n" + "!" * 40)
        print("DEBUG: FORCING AN EXTRA TIE (ALL DRIVERS)")
        ...
        # Ensure the other drivers don't participate 
        for driver_name in overall_standings:
            if driver_name != leader_1 and driver_name != leader_2:
                overall_standings[driver_name] = 10
        print("!" * 60 + "\n")
